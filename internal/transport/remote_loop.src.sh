...
# ── ping target + firmware (LUCI regs 5/6) + cpu/kernel facts ──
ph='__PING_HOST__';
...
# ── pg(): one ICMP ping, echo the avg RTT (ms) via shared $o, or "-" on failure ──
pg() {
...
while :; do
  ...
  if [ "$dg" = 1 ]; then
    ...
    pgc=$((pgc-1));
    if [ $pgc -le 0 ]; then pg "$cip"; pcl=$o; pg "$gw"; pgw=$o; pg "$ph"; pnt=$o; pgc=3; else pcl=-; pgw=-; pnt=-; fi;
    echo @@s;
  ...
  [ $bw -gt 0 ] && { bw=$((bw-1)); i=0; };
  # idle when reg 51 reports position 0; slows the read timeout from 1s to 3s
